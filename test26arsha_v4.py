...
async def cmd_install_util(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title="[편의기능]", color=0x2b2d31)
    embed.add_field(name="🗡️ 강화 및 공/방 계산", value="\u200b", inline=False)
    embed.add_field(name="💰 펄의상 및 거래소 효율 계산", value="\u200b", inline=False)
    embed.add_field(name="📋 기타", value="\u200b", inline=False)
    await interaction.channel.send(embed=embed, view=SetupUtilityView())
    await interaction.response.send_message("✅ 편의기능 패널 설치 완료", ephemeral=True); schedule_ephemeral_delete(interaction)
